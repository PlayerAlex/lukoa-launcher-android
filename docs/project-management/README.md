...
- 分支：`main`（2026-09-03 起重新成为唯一开发分支，此前的 `codex/termux-wake-foreground-resume` 已快进合并并删除）
- 当前版本：`v1.0` 正式版，发布于 `2026-08-11`，`versionCode 286`
- 主要风险：
  - 上传限制工具会修改 SillyTavern 的上传中间件源码，需要在真机验证大文件内存占用、酒馆更新后的补丁状态和失败恢复。
  - 依赖修复、主题重置等修复工具尚未在多个 SillyTavern 版本上做过系统验证。
  - 不同品牌手机的后台保活策略与 Termux 小窗/分屏兼容性差异。

如果这里的版本描述和代码、`CHANGELOG.md`、Git tag 不一致，以代码和最新的 `session-notes` 为准。
...
如果后面测试样本越来越多，可以继续在这个目录下新增 `test-reports/`、`release-checklists/` 之类的子目录。现阶段维持现有文件即可。
